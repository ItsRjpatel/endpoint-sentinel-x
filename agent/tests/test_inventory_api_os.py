"""
Unit tests for the OS inventory API client.
"""
...
from api.inventory import submit_os
...
def _make_response(status_code: int, json_body: dict | None = None) -> httpx.Response:
    ...
    return httpx.Response(
        ...
        request=httpx.Request("POST", "http://testserver/api/v1/inventory/operating-system"),
    )
...
class TestSubmitOSSuccess:
    ...
    def test_accepted_response_does_not_raise(self):
        ...
        with patch("api.inventory._get_http_client") as mock_get_client:
            ...
            submit_os(body=_SAMPLE_BODY, config=cfg)
    ...
    def test_correct_url_is_called(self):
        ...
        assert called_url == "http://sentinel-backend/api/v1/inventory/operating-system"
